...
class hyb(object):
    ...
    def train(self, Niteration = 5000, print_message=True):
        self.maps = []
        T0 = 0.1
        ...
        obj_curr = (FP + FP)/self.N +self.alpha*(len(prs_curr) + len(nrs_curr))+ self.beta * sum(~covered_curr)/self.N
        self.actions = []
        for iter in range(Niteration):
            if iter >0.5 * Niteration:
            ...
            if obj_new < self.maps[-1][1]:
                ...
                print('\n**  max at iter = {} ** \n {}(obj) = {}(error) + {}(nrules) + {}(exp)\n accuracy = {}, explainability = {}\n perror = {}, nerror = {}, oerror = {}, berror = {}\n '.format(iter,round(obj_new,3),(FP+FN)/self.N, self.alpha*(len(prs_new) + len(nrs_new)), self.beta*sum(~covered_new)/self.N, (TP+TN+0.0)/self.N,sum(covered_new)/self.N,perror,nerror,oerror,berror ))
                self.maps.append([iter,obj_new,prs_new,nrs_new])
            
            if print_message:
                perror, nerror, oerror, berror = self.diagnose(pcovered_new,ncovered_new,overlap_new,covered_new,Yhat_new)
                if print_message:
                    print('\niter = {}, alpha = {}, {}(obj) = {}(error) + {}(nrules) + {}(exp)\n accuracy = {}, explainability = {}\n perror = {}, nerror = {}, oerror = {}, berror = {}\n '.format(iter,round(alpha,2),round(obj_new,3),(FP+FN)/self.N, self.alpha*(len(prs_new) + len(nrs_new)), self.beta*sum(~covered_new)/self.N, (TP+TN+0.0)/self.N,sum(covered_new)/self.N,perror,nerror,oerror,berror ))
